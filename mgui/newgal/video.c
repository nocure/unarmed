/*
**  $Id: video.c 7596 2007-09-17 10:15:58Z houhuihua $
**
...
extern VideoBootStrap RGB32_bootstrap;
...
GAL_VideoDevice *GAL_GetVideo(const char* driver_name)
{
    ...
    index_char=strrchr(driver_name,':');
    if (index_char) {
        index=atoi(index_char+1);
    }
    else {
        index = 0;
    }
...
GAL_Surface *Slave_VideoInit(const char* driver_name, const char* mode)
{
    ...
    if (video == NULL) {
        dbgout ("NEWGAL: Does not find the slave video engine: %s.\n", driver_name);
        return NULL;
    }
    ...
    if ( video->VideoInit(video, &vformat) < 0 ) {
        dbgout ("NEWGAL: Can not init the slave video engine: %s.\n", driver_name);
    ...
    surface = Slave_CreateSurface (video, 0, 0, vformat.BitsPerPixel,
                vformat.Rmask, vformat.Gmask, vformat.Bmask, 0);
    if (!surface) {
        dbgout ("NEWGAL: Create slave video surface failure.\n");
        return NULL;
    }
