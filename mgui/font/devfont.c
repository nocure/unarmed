...
#else
		dbgout("Sorry, you don't open freetype font engine.\n");
        return FALSE;
#endif
...
#else
		dbgout("Sorry, you don't open qpf font engine.\n");
		return FALSE;
#endif
	} else {
		dbgout("font type error, it must be freetype or qpf\n");
		return FALSE;
	}
...
void GUIAPI DestroyDynamicDevFont (DEVFONT **devfont)
{
	...
	if (!fontGetTypeNameFromName ((*devfont)->name, type_name)) {
		dbgout ("Destroy device font type error.\n");
		return;
	}
