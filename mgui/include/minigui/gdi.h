...
/*
 * $Id: gdi.h 9941 2008-04-01 05:14:01Z xwyan $
 *
...
{
//    pthread_mutex_t lock;		// TODO: restore it
    /**
 ...
 * \brief Gets the handle to the window by a handle to the DC.
 *
 * \param hdc The handle to the DC.
 *
 * \return The handle to the window; HWND_NULL when the DC is
 *          a memory dc or HDC_SCREEN.
 *
 * \sa GetDC
 */
 ...
 * accessed, and the function will return the effective \a width, \a height
 * and \a pitch of the DC. The access beyond effective width and height
 * will be invalid.
 *
 ...
 *        in bytes.
 * \return The bits pointer to the upper-left corner of the effective
...
/**
 * \fn GAL_Overlay* GUIAPI CreateYUVOverlay (int width, int height, \
 ...
 * \param format The expected video overlay format, can be one of
 ...
 * \note Only defined for _USE_NEWGAL.
 *
 * \sa GAL_LockYUVOverlay, GAL_FreeYUVOverlay,
 * http://www.webartz.com/fourcc/indexyuv.htm
 */
 ...
 * different from that of the overlay, but currently only 2x scaling is
 * supported. And note that the \a dstrect is in screen coordinate system.
 *
 ...
 */
MG_EXPORT void GUIAPI DisplayYUVOverlay (GAL_Overlay* overlay,
                const RECT* dstrect);
 ...
 * \brief Sets the gamma translation table for the red, green, and blue
 *        channels of the video hardware.
 *
 * This function sets the gamma translation table for the red, green, and
 * blue channels of the video hardware. Each table is an array of 256 16-bit
 * quantities, representing a mapping between the input and output for that
 * channel. The input is the index into the array, and the output is the
 * 16-bit gamma value at that index, scaled to the output color precision.
 *
 ...
 *
 * This function retrieves the current values of the gamma translationn tables.
 * You must pass in valid pointers to arrays of 256 16-bit quantities.
 * Any of the pointers may be NULL to ignore that channel.
 *
 * \param red Pointers to the array of gamma translation tables for
 *        the red channel.
 * \param green Pointers to the array of gamma translation tables for
 *        the green channel.
 * \param blue Pointers to the array of gamma translation tables for
 *        the blue channel.
 *
 * \return If the call succeeds, it will return 0. If the display driver or
 *         hardware does not support gamma translation, or otherwise fails,
...
/**
 * \fn MG_EXPORT HPALETTE GUIAPI GetDefaultPalette (void);
 ...
 * \sa RGB2Pixel
 */
MG_EXPORT void GUIAPI Pixel2RGB (HDC hdc, gal_pixel pixel,
                Uint8* r, Uint8* g, Uint8* b);
 ...
 * \sa RGB2Pixel
 */
MG_EXPORT void GUIAPI Pixel2RGBA (HDC hdc, gal_pixel pixel,
                Uint8* r, Uint8* g, Uint8* b, Uint8* a);
...
MG_EXPORT gal_pixel GUIAPI RGB2Pixel (HDC hdc, int r, int g, int b);
...
/**
 * \fn BOOL GUIAPI PolygonGenerator (void* context, \
                const POINT* pts, int vertices, CB_POLYGON cb)
...
/**
 * \fn BOOL GUIAPI BitmapDDAScaler (void* context, \
 ...
 */
MG_EXPORT BOOL GUIAPI BitmapDDAScaler (void* context,
                const BITMAP* src_bmp, int dst_w, int dst_h,
                CB_GET_LINE_BUFF cb_get_line_buff,
                CB_LINE_SCALED cb_line_scaled);

/**
 * \fn BOOL GUIAPI BitmapDDAScaler2 (void* context, \
                const BITMAP* src_bmp, int dst_w, int dst_h, \
                CB_GET_LINE_BUFF cb_get_line_buff, \
                CB_LINE_SCALED cb_line_scaled)
 * \brief A bitmap scaler using DDA algorithm.
 ...
 */
MG_EXPORT BOOL GUIAPI BitmapDDAScaler2 (void* context,
                const BITMAP* src_bmp, int dst_w, int dst_h,
                CB_GET_LINE_BUFF cb_get_line_buff,
                CB_LINE_SCALED cb_line_scaled);
 ...
 *       if _USE_NEWGAL defined, and the current raster operation was set
 *       by \a SetRasterOperation for the DC will override the alpha value of
 *       color key if ROP is not ROP_SET.  If _USE_NEWGAL is not defined,
 *       this function will not return anything, and you can not specify the
 ...
 * \return TRUE on success, otherwise FALSE.
 *
 * \note You can specify the alpha value or the color key of the BITMAP
 *       object if _USE_NEWGAL defined, and the current raster operation
 *       was set by \a SetRasterOperation for the DC will override the alpha
 *       value or the color key if ROP is not ROP_SET. If _USE_NEWGAL not
 *       defined, this function will not return anything, and you can not
 *       specify the alpha value and the color key of the BITMAP object.
 *
 ...
 *
 * \note If _USE_NEWGAL defined, the alpha and color key settings of
 *       the source DC will come into play.
 *
 ...
 * \brief Scales a BITMAP object into another BITMAP object by using
 *        DDA algorithm.
 *
...
/**
 * \fn BOOL GUIAPI SetPixelInBitmap (const BITMAP* bmp, \
 ...
 */
MG_EXPORT BOOL GUIAPI SetPixelInBitmap (const BITMAP* bmp,
                int x, int y, gal_pixel pixel);
...
MG_EXPORT void GUIAPI SetRect (RECT* prc, int left, int top,
...
typedef struct _CHARSETOPS
{
	...
	/** The pointer to get the length of the first character function. */
    int (*len_first_char) (const unsigned char* mstr, int mstrlen);

	/** The pointer to get character offset function. */
    unsigned int (*char_offset) (const unsigned char* mchar);

	/** The pointer to get character type function. */
    unsigned int (*char_type) (const unsigned char* mchar);

	/** The pointer to get character number in the string function. */
    int (*nr_chars_in_str) (const unsigned char* mstr, int mstrlen);

	/** The pointer to judge the \a charset is belong to the character set
	 *  function.
	 */
    int (*is_this_charset) (const unsigned char* charset);

	/** The pointer to get  the length of the first substring. */
    int (*len_first_substr) (const unsigned char* mstr, int mstrlen);

	/** The pointer to get next word in the specitied length string function. */
    const unsigned char* (*get_next_word) (const unsigned char* mstr,
                int strlen, WORDINFO* word_info);

	/** The pointer to get the position of the first character in the
	 *  specified length string function.
	 */
    int (*pos_first_char) (const unsigned char* mstr, int mstrlen);

#ifdef _UNICODE_SUPPORT
	/** The pointer to convert \a mchar to 32 bit UNICODE function. */
    UChar32 (*conv_to_uc32) (const unsigned char* mchar);

	/** The pointer to convert \a wc to multily byte character function. */
...
typedef struct _FONTOPS
{
	...
	/* The pointer to preproccess start string output fuction, call this
	 * function before output a string, can be NULL.
	 */
    void (*start_str_output) (LOGFONT* logfont, DEVFONT* devfont);

	/** The pointer to get character bound box function, can be NULL. */
    int (*get_char_bbox) (LOGFONT* logfont, DEVFONT* devfont,
            const unsigned char* mchar, int len,
            int* px, int* py, int* pwidth, int* pheight);

	/** The pointer to get character advance function, can be NULL. */
    void (*get_char_advance) (LOGFONT* logfont, DEVFONT* devfont,
            const unsigned char* mchar, int len, int* px, int* py);

	/** The pointer to instance new device font function, can be NULL. */
    DEVFONT* (*new_instance) (LOGFONT* logfont, DEVFONT* devfont,
            BOOL need_sbc_font);

	/** The pointer to delete instance of device font function, can be NULL. */
...
struct _DEVFONT
{
    ...
    const FONTOPS*         font_ops;

	/** The pointer to character set operation structure. */
    CHARSETOPS*      charset_ops;

	/** The pointer to single charset set device font. */
    struct _DEVFONT* sbc_next;

	/** The pointer to multiply charset set device font. */
 ...
 * \sa mg_FT_LcdFilter
 */
MG_EXPORT BOOL ft2SetLcdFilter (LOGFONT* logfont, mg_FT_LcdFilter filter);

#endif

#if (defined(_QPF_SUPPORT) || defined(_FT2_SUPPORT) || defined(_TTF_SUPPORT))

/**
 * \fn BOOL GUIAPI LoadDevFontFromFile (const char* devfont_name, \
 ...
 *
 * \param devfont The pointer to pointer of DEVFONT structure.
 *
 * \return TRUE on success, FALSE on error.
 *
 * \sa GetNextDevFont, DestroyDynamicDevFont
 */
MG_EXPORT BOOL GUIAPI LoadDevFontFromFile (const char *devfont_name,
        const char *file_name, DEVFONT **devfont);
...
/**
 * \fn void GUIAPI GetTextExtent (HDC hdc, \
 ...
 */
MG_EXPORT void GUIAPI GetTextExtent (HDC hdc,
                const char* spText, int len, SIZE* pSize);

/**
 * \fn void GUIAPI GetTabbedTextExtent (HDC hdc, \
                const char* spText, int len, SIZE* pSize)
 ...
 * \sa GetTextExtent
 */
MG_EXPORT void GUIAPI GetTabbedTextExtent (HDC hdc,
                const char* spText, int len, SIZE* pSize);
 ...
 * at \a (x,y). This function returns the width of text. The output will
 * be affected by the default tab size if there are some TAB characters in
 * the text.
 *
 * \param hdc The device context.
 * \param x x,y: The output start position.
 ...
 * array \a pTabPositions of tab-stop positions.
 *
 * \param hdc The device context.
 * \param x x,y: The output start position.
 * \param y x,y: The output start position.
 ...
 *
 */
MG_EXPORT void TerminateSlaveScreen (HDC hdc);
/**
...
void GUIAPI Expand16CBitmap (HDC hdc, BYTE* bits, Uint32 pitch,
                const BYTE* my_bits, Uint32 my_pitch,
 ...
 */
/**
 * \defgroup rlp Run Length Encoded Pattern (RLP) Support
 *
 */

/**
 * \fn void PutRLP(HDC hdc, int x, int y, BYTE* pattern)
 * \brief Displays an RLP.
 *
 *  This function displays an RLP image.
 *
 * \param hdc The device context.
 * \param x (x,y) The output start position.
 * \param y (x,y) The output start position.
 * \param memory address to The RLP array.
 *
 * \return None.
 *
 */
MG_EXPORT void PutRLP(HDC hdc, int x, int y, BYTE* pattern);

/**
 * \fn int CreateBitmapFromRLP(PBITMAP pBmp, BYTE* pattern)
 * \brief Creates a device-dependent bitmap from an RLP.
 *
 *  This function creates a device-dependent bitmap from an RLP.
 *
 * \param pBitmap The pointer to the BITMAP object.
 * \param memory address to The RLP array.
 *
 ...
 *
 */
MG_EXPORT int CreateBitmapFromRLP(PBITMAP pBmp, BYTE* pattern);

#else

MG_EXPORT int GUIAPI ExpandMyBitmap (HDC hdc, const MYBITMAP* pMyBitmap,
                const RGB* pal, PBITMAP pBitmap);
MG_EXPORT void GUIAPI ExpandMonoBitmap (HDC hdc, int w, int h,
                const BYTE* my_bits, int my_pitch, int bits_flow,
                BYTE* bitmap, int pitch, int bg, int fg);
MG_EXPORT void GUIAPI Expand16CBitmap (HDC hdc, int w, int h,
                const BYTE* my_bits, int my_pitch, int bits_flow,
                BYTE* bitmap, int pitch, const RGB* pal);
MG_EXPORT void GUIAPI Expand256CBitmap (HDC hdc, int w, int h,
                const BYTE* my_bits, int my_pitch, int bits_flow,
                BYTE* bitmap, int pitch, const RGB* pal);
MG_EXPORT void GUIAPI CompileRGBBitmap (HDC hdc, int w, int h,
                const BYTE* my_bits, int my_pitch, int bits_flow,
