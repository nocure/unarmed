/*
** $Id: gdi.c 8281 2007-11-27 09:18:50Z weiym $
**
...
/* TODO: replace pthread_mutex_t with tx correspoding mutex */
...
BOOL InitGDI (void)
{
    if (!InitTextBitmapBuffer ()) {
        dbgout ("GDI: Can not initialize text bitmap buffer!\n");
        goto error;
    }
    ...
    if (!InitIncoreRBFonts ()) {
        dbgout ("GDI: Can not initialize incore RBF fonts!\n");
        goto error;
    }
#endif
    ...
    if (!InitSysFont ()) {
        dbgout ("GDI: Can not create system fonts!\n");
        goto error;
    }
...
void GUIAPI UnlockDC (HDC hdc)
{
    ...
    if (!dc_IsMemDC (pdc)) ShowCursorForGDI (TRUE, pdc);
...
HDC GUIAPI InitSlaveScreen (const char* name, const char* mode)
{
    ...
    if ((surface = Slave_VideoInit (name, mode))) {
        ...
        free (pmem_dc);
        dbgout ("NEWGDI: Can not init the slave screen: %s (%s)\n",
                        name, mode);
        return HDC_INVALID;
    }
}
