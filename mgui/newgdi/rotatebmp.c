/*
** $Id: rotatebmp.c 7954 2007-10-24 11:31:57Z weiym $
**
...
#endif
    dst->bmAlphaPixelFormat = src->bmAlphaPixelFormat;
    dst->bmHeight = 1;
				    ...
				    fixed sx[4], fixed sy[4])
{
    ...
    sx[tl] = xofs;
    sy[tl] = yofs;
    sx[tr] = xofs + fixmul(w, fix_cos);
    sy[tr] = yofs + fixmul(w, fix_sin);
    sx[bl] = xofs - fixmul(h, fix_sin);
    sy[bl] = yofs + fixmul(h, fix_cos);

    sx[br] = sx[tr] + sx[bl] - sx[tl];
    sy[br] = sy[tr] + sy[bl] - sy[tl];
                   ...
                   BOOL h_flip, BOOL v_flip)
{
    ...
    fixangle = ftofix(((angle/64) % 360) * 256/360.);
