/*
** $Id: polygon.c 9932 2008-03-28 06:46:59Z houhuihua $
**
...
BOOL GUIAPI PolygonGeneratorEx (void* context, const POINT* pts, int vertices, CB_POLYGON cb, RECT* rc_output)
{
    int c;
    int top = INT_MAX;
    int bottom = INT_MIN;
    ...
    if (rc_tmp.left == rc_tmp.right) {
        ...
        if (pdc->brush_type == BT_SOLID) {
            if (is_mv)
                ...
                PolygonGeneratorEx(pdc, points, vertices, _dc_draw_hline_clip, &pdc->rc_output);
        }
        else {
            if (is_mv)
                MonotoneVerticalPolygonGenerator (pdc, points,
