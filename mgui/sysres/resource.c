                ...
                int ckey_x, int ckey_y)
{
    ...
    if (GetMgEtcValue ("bitmapinfo", szItemName,
            szValue, MAX_NAME) < 0 ) {
        dbgout ("LoadSystemBitmapEx: Get bitmap file name error!\n");
        return FALSE;
    }

    if (GetMgEtcValue ("bitmapinfo", "bitmappath",
            szPathName, MAX_PATH) < 0 ) {
        dbgout ("LoadSystemBitmapEx: Get bitmap path error!\n");
        return FALSE;
    }
    ...
    if (LoadBitmap (HDC_SCREEN, pBitmap, szPathName) < 0) {
        dbgout ("LoadSystemBitmapEx: Load bitmap error: %s!\n",
                        szPathName);
        return FALSE;
    }
...
HICON GUIAPI LoadSystemIcon (const char* szItemName, int which)
{
    ...
    if (GetMgEtcValue ("iconinfo", szItemName,
            szValue, MAX_NAME) < 0 ) {
        dbgout ("LoadSystemIcon: Get icon file name error!\n");
        return 0;
    }

    if (GetMgEtcValue ("iconinfo", "iconpath",
            szPathName, MAX_PATH) < 0 ) {
        dbgout ("LoadSystemIcon: Get icon path error!\n");
        return 0;
    }
    ...
    if ((hIcon = LoadIconFromFile (HDC_SCREEN, szPathName, which)) == 0) {
        dbgout ("LoadSystemIcon: Load icon error: %s!\n", szPathName);
        return 0;
    }
                ...
                int ckey_x, int ckey_y)
{
    ...
    if (!bmpdata) {
        dbgout ("LoadSystemBitmapEx: Get bitmap data error!\n");
        return FALSE;
    }

    if ((nc = LoadBitmapFromMem (HDC_SCREEN, pBitmap,
                              bmpdata, len, "bmp"))) {
        dbgout ("LoadSystemBitmapEx: Load bitmap "
                        "error: %p, %d: %d!\n", bmpdata, len, nc);
        return FALSE;
    }
    ...
    if (ckey_x >= 0 && ckey_x < pBitmap->bmWidth
            && ckey_y >= 0 && ckey_y < pBitmap->bmHeight) {
        pBitmap->bmType = BMP_TYPE_COLORKEY;
        pBitmap->bmColorKey = GetPixelInBitmap (pBitmap, ckey_x, ckey_y);
    }
    ...
    for (i = 0; i < NR_BMPS; i++) {
        if (sz_bmps [i] > 0) {
            if (LoadBitmapFromMem (HDC_SCREEN, SystemBitmap + i, bmp,
                                sz_bmps [i], "bmp")) {
                dbgout ("InitSystemRes: error when loading %d "
                                "system bitmap.\n", i);
                return FALSE;
            }
            bmp += sz_bmps [i];
