...
{
#if 0
	yl_timerSetCountDown(timeout);
    while( !yl_uartIsDataReady()) {
    	if (!yl_timerGetCountDown()) {
    		return -2;
    	}
    }
//    if (!yl_uartIsDataReadyWithTimeout(timeout)) {
//    	return -2;
//    }
	return yl_uartGetReceivedData();
#endif
	return getch();
}
...
int xmodemReceive(unsigned char *dest, int destsz)
{
	...
	for(;;) {
		...
		while(1) { // wsw 2008-0815 modified, wait to receive forever
