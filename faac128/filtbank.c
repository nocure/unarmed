...
#ifdef FIXED_POINT_MDCT	// ww 2011-0908 added fixed point mdct routine
#include "fixed_point_mdct/fip_mdct.h" // ww 2011-0908 added for fixed point mdct
#endif
...
void FilterBank(faacEncHandle hEncoder,
                ...
                int overlap_select)
{
    double *p_o_buf, *first_window=0, *second_window=0;
    double *transf_buf;
    int k, i;
...
{
    int i;
    real_t data_tmp[LENGTH];

    for(i=0;i<N;i++)
    {
    	data_tmp[i] = REAL_CONST(data[i]);
    }

    switch (N) {
		case BLOCK_LEN_SHORT * 2:
			fixp_mdct_short(data_tmp);
			break;

		case BLOCK_LEN_LONG * 2:
			fixp_mdct_long(data_tmp);
			break;
    }

    for(i=0;i<N;i++)
    {
    	// faad_mdct has a scale of (4.0/N)
    	// it's required to *2 for post-twiddle output
    	data[i] = RTOD(data_tmp[i])/(4.0/N)*2;
    }
