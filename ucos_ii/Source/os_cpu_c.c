...
// ww 2012-0815 added --> make ExtPtr zero
/*
 * When on task deletion, address in ptcb->OSTCBExtPtr is still available,
 * But It was actually deleted by OSTaskDelHook() in OSTaskDel, OSTaskDel then
 * call OS_Sched to swtich task, which calls OSTaskSwHook in OSCtxSw, which will
 * try to OS_CPU_FP_Save in OSTaskSwHook, in which stores VPF register to
 * ptcb->OSTCBExtPtr that is no longer exist. so ptcb->OSTCBExtPtr must not be accessible.
 */
			ptcb->OSTCBExtPtr=0;
// ww 2012-0815 added <--
        }
...
*********************************************************************************************************
*/
// ww 2012-0814 addeded --> Allow task to 'EXIT'
static void OSTaskExitPoint(void) {
	OSTaskDel(OS_PRIO_SELF);
}
// ww 2012-0814 addeded <--
...
// ww 2012-0814 modified --> Allow task to 'EXIT'
//    *(--stk)  = (INT32U)0x14141414L;             /* R14 (LR)                                           */
    *(--stk)  = (INT32U)OSTaskExitPoint;			 /* R14 (LR) Exit point */
// ww 2012-0814 modified <--
