...
//printf("read_cmov 1 %lld\n", quicktime_position(file));
			...
			if(strcmp(data, "zlib"))
			{
				printf("read_cmov: compression '%c%c%c%c' not zlib.  Giving up and going to a movie.\n",
					data[0],
					data[1],
					data[2],
					data[3]);
...
/* Read compressed data */
			...
			{
				printf("read_cmov: FYI compressed_size=%d uncompressed_size=%d\n",
					compressed_size,
					uncompressed_size);
			}
...
/* Trick library into reading temporary buffer for the moov */
			...
			file->preload_buffer = data_out;		// TODO: restore
