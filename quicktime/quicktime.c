...
/* find the moov atom in the old file */
	...
	{
		printf("quicktime_make_streamable");
		return 1;
	}
...
void quicktime_set_framerate(quicktime_t *file, double framerate)
{
	...
	if(!file->wr)
	{
		printf("quicktime_set_framerate shouldn't be called in read mode.\n");
		return;
	}
...
int quicktime_set_audio_position(quicktime_t *file, int64_t sample, int track)
{
	...
	if(track < file->total_atracks)
	{
		...
		printf("quicktime_set_audio_position: track >= file->total_atracks\n");
...
int quicktime_set_video_position(quicktime_t *file, int64_t frame, int track)
{
	...
	{
		printf("quicktime_set_video_position: frame=%lld track=%d >= file->total_vtracks %d\n", 
			frame,
			track,
			file->total_vtracks);
	...
	if(!result)
	{
		if(rd)
		{
			if(quicktime_read_info(new_file))
			{
				...
				printf("quicktime_open: error in header\n");
				new_file = 0;
			}
		}
