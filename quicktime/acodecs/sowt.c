...
static int swap_bytes(char *buffer, long samples, int channels, int bits)
{
	...
	if(byte_order()) return 0;		/* if little endian, dont swap */
	...
	switch(quicktime_audio_bits(file, track))
	{
		case 8:
			if(output_i && !result)
			{
				...
				{
					if(file->use_avi)
						output_i[i] = (int16_t)((unsigned char)codec->work_buffer[j]) - 0x7fff;
					else
						output_i[i] = (int16_t)codec->work_buffer[j];
					j += step;
				}
			}
			else
		...
		case 24:
			...
			if(output_f && !result)
			{
				for(i = 0, j = channel * 3; i < samples; i++)
				{
					output_f[i] = (float)((((int)codec->work_buffer[j + 2]) << 16) | 
									(((unsigned char)codec->work_buffer[j + 1]) << 8) |
									((unsigned char)codec->work_buffer[j])) / 0x7fffff;
					j += step;
				}
			}
			break;
		...
		for(i = 0; i < track_map->channels; i++)
		{
			switch(quicktime_audio_bits(file, track))
			{
				...
				case 16:
					...
					{
						sample = input_i[i][j];
						codec->work_buffer[j * step + i * 2 + 1] = ((unsigned int)sample & 0xff00) >> 8;
						codec->work_buffer[j * step + i * 2] = ((unsigned int)sample) & 0xff;
					}
					break;
				case 24:
					for(j = 0; j < samples; j++)
					{
						sample = input_i[i][j];
						codec->work_buffer[j * step + i * 3 + 2] = ((unsigned int)sample & 0xff00) >> 8;
						codec->work_buffer[j * step + i * 3 + 1] = ((unsigned int)sample & 0xff);
						codec->work_buffer[j * step + i * 3] = 0;
					}
					break;
			}
		}
			...
			switch(quicktime_audio_bits(file, track))
			{
				...
				case 24:
					for(j = 0; j < samples; j++)
					{
						...
						codec->work_buffer[j * step + i * 3 + 2] = ((unsigned int)sample & 0xff0000) >> 16;
						codec->work_buffer[j * step + i * 3 + 1] = ((unsigned int)sample & 0xff00) >> 8;
						codec->work_buffer[j * step + i * 3] = ((unsigned int)sample) & 0xff;
					}
					break;
			}
		}
