...
int new_vcodec(quicktime_video_map_t *vtrack)
{
	...
	if(result)
	{
		printf("new_vcodec: couldn't find codec for \"%c%c%c%c\"\n",
			compressor[0],
			compressor[1],
			compressor[2],
			compressor[3]);
...
int new_acodec(quicktime_audio_map_t *atrack)
{
	...
	if(result)
	{
		printf("new_acodec: couldn't find codec for \"%c%c%c%c\"\n",
			compressor[0],
			compressor[1],
			compressor[2],
			compressor[3]);
	...
	int track)
{
	...
	{
		printf("quicktime_decode_video: track %d out of range %d - %d\n",
			track,
			0,
			file->total_vtracks);
		return 1;
