...
NeAACDecHandle NEAACDECAPI NeAACDecOpen(void)
{
    ...
    if ((hDecoder = (NeAACDecStruct*)faad_malloc(sizeof(NeAACDecStruct))) == NULL)  // Allocate memory buffer for decode
        return NULL;

    memset(hDecoder, 0, sizeof(NeAACDecStruct));	// Initial decode struct buffer.
...
//            return 0;
        hDecoder->config.options = config->options;
// ww 2010-0927 modified <--
        /* OK */
        return 1;
    }
                              ...
                              unsigned char *channels)
{
    ...
    hDecoder->sf_index = get_sr_index(hDecoder->config.defSampleRate);		//Sample rate index.
    ...
    *channels = 1;
    ...
    if (buffer != NULL)
    {
        ...
        } else if (faad_showbits(&ld, 12) == 0xfff) {		// sync word?
            hDecoder->adts_header_present = 1;

            adts.old_format = (hDecoder->config.options&OLD_ADTS_FORMAT)?1:0;
            adts_frame(&adts, &ld);		//Get herder and frame information.

            hDecoder->sf_index = adts.sf_index;
            hDecoder->object_type = adts.profile + 1;

            *samplerate = get_sample_rate(hDecoder->sf_index);		// Get sample rate.
            *channels = (adts.channel_configuration > 6) ?			// Get channel.
...
// ww 2010-0927 modified -->
#if defined(__sparc__) && defined(IMDCT_YLHW_SUPPORTED)
   	if (hDecoder->config.options&YL_HW_IMDCT) {
   		yl_imdctInit();
   		yl_imdctEnable(1);
   	}
#endif
	hDecoder->fb = filter_bank_init(hDecoder->frameLength,(hDecoder->config.options&YL_HW_IMDCT)?1:0);
// ww 2010-0927 modified <--
...
// ww 2010-0927 added -->
#if defined(__sparc__) && defined(IMDCT_YLHW_SUPPORTED)
    if (hDecoder->config.options&YL_HW_IMDCT) yl_imdctEnable(0);
#endif
// ww 2010-0927 added <--
#ifdef PROFILE
...
//    uint32_t startbit=0, endbit=0, payload_bits=0;
    ...
    memset(hDecoder->internal_channel, 0, MAX_CHANNELS*sizeof(hDecoder->internal_channel[0]));	// 64 bytes
...
#endif
        raw_data_block(hDecoder, hInfo, &ld, &hDecoder->pce, hDecoder->drc);		// Process row data.
#ifdef DRM
    }
#endif
...
// ww 2010-0927 modified -->
    if ((channels == 5 || channels == 6) && (hDecoder->config.options&DOWN_MATRIX))
// ww 2010-0927 modified <--
    {
