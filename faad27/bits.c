...
/*INLINE*/ uint32_t faad_getbits(bitfile *ld, uint32_t n DEBUGDEC)
{
    uint32_t ret;
...
INLINE uint8_t faad_get1bit(bitfile *ld DEBUGDEC)
{
    uint8_t r;
...
/*INLINE*/ uint32_t faad_getbits_rev(bitfile *ld, uint32_t n
                                        DEBUGDEC)
{
    uint32_t ret;
...
void faad_initbits(bitfile *ld, const void *_buffer, const uint32_t buffer_size)
{
    ...
    if (ld->bytes_left >= 4)		// at least remaining 32bits data ?
    {
        tmp = getdword((uint32_t*)ld->buffer);	// get source data from pointer and exchange high byte with low byte.
        ld->bytes_left -= 4;					//
    } else {
    ...
    ld->bits_left = 32;		// 32-bit data format.
