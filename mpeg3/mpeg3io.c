...
// Shift existing buffer forward and calculate amount of new data needed.
		...
		fread(fs->buffer, remainder_start, fs->fd);
...
//printf("mpeg3io_read_buffer 2 %llx %llx\n", fs->buffer_position, ftell(fs->fd));
		fs->buffer_size = fread(fs->buffer, MPEG3_IO_SIZE, fs->fd);
...
{
#if 0
	struct _stati64 file_st, device_st;
    struct mntent *mnt;
	FILE *fp;

	if(_stati64(path, &file_st) < 0)
	{
	...
	{
		if(_stati64(mnt->mnt_fsname, &device_st) < 0) continue;
