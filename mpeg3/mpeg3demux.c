...
/* Skip pes packet length */
	...
	{
/* Dump private data! */
		printf("stream_id == MPEG3_PRIVATE_STREAM_2\n");
		packet_skip(demuxer, demuxer->raw_size - demuxer->raw_offset);
		return 0;
	}
	else
	...
	{
    	printf("unknown stream_id in pes packet");
		return 1;
	}
	return 0;
}
...
//printf(__FUNCTION__ " data_size=%x decryption_offset=%x\n", demuxer->data_size, decryption_offset);
		if(mpeg3_decrypt_packet(title->fs->css,
			...
			decryption_offset))
		{
			printf("handle_scrambling: Decryption not available\n");
			return 1;
		}
	}
...
int mpeg3demux_open_title(mpeg3_demuxer_t *demuxer, int title_number)
{
		...
		title_number >= 0)
	{
		...
		if(mpeg3io_open_file(title->fs))
		{
			...
			printf("mpeg3demux_open_title %s: %s", title->fs->path, strerror(errno));
		}
		else
		{
	...
	{
		printf("mpeg3demux_open_title title_number = %d\n",
			title_number);
	}
