...
// ww 2018-0604 added -->
// to avoid conflict and re-define error of some types in time.h.
...
static void print_fps (int final)
{
    ...
    printf ("%d frames in %d.%02d sec (%d.%02d fps), "
...
static void ps_loop (void)
{
    ...
    do {
	end = buffer + fread (buffer, BUFFER_SIZE, in_file);
...
#define PACKETS (BUFFER_SIZE / 188)
    ...
    do {
	packets = fread (buffer, PACKETS * 188, in_file);
	for (i = 0; i < packets; i++) {
	    buf = buffer + i * 188;
	    end = buf + 188;
	    if (buf[0] != 0x47) {
		printf ("bad sync byte\n");
		exit (1);
	    }
...
static void es_loop (void)
{
    ...
    do {
	size = fread (buffer, BUFFER_SIZE, in_file);
