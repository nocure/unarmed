...
static int
cp950_mbtowc (wchar_t *pwc, const unsigned char *s, int n)
{
  unsigned char c = *s;
  /* Code set 0 (ASCII) */
  if (c < 0x80)
  ...
  if (c >= 0x81 && c < 0xff) {
    ...
    if (c == 0xf9) {
      int ret = cp950ext_mbtowc(pwc,s,2);
      if (ret != RET_ILSEQ)
        return ret;
    }
  }
...
static int
cp950_wctomb (unsigned char *r, wchar_t wc, int n)
{
  unsigned char buf[2];
  int ret;
